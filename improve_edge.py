...
def optimizePoints(p0_list, p1_list, edges):
    ...
    for cur_p0 in p0_list:
        for cur_p1 in p1_list:
            ...
            #print("score:",score,"xcoords:",xcoordinates,", ycoords:", ycoordinates)
            if score > best_score:
                best_score = score
                best_p0 = cur_p0
                best_p1 = cur_p1
...
def segsPlot(p0, p1, best_p0, best_p1, edges, image, fig_size = (20,10)):
    ...
    min_x = min(int(np.min([best_p0[0], best_p1[0]])-10), int(np.min([p0[0], p1[0]])-10))
    min_y = min(int(np.min([best_p0[1], best_p1[1]])-10), int(np.min([p0[1], p1[1]])-10))
    max_x = max(int(np.max([best_p0[0], best_p1[0]])+10), int(np.max([p0[0], p1[0]])+10))
    max_y = max(int(np.max([best_p0[1], best_p1[1]])+10), int(np.max([p0[1], p1[1]])+10))
    axs[0].imshow(image[min_y:max_y, min_x:max_x, :])
    axs[0].plot([p0[0]-min_x, p1[0]-min_x], [p0[1]-min_y, p1[1]-min_y], c = 'r')
    axs[0].plot([best_p0[0]-min_x, best_p1[0]-min_x], [best_p0[1]-min_y, best_p1[1]-min_y], c = 'y')
    axs[1].imshow(toRGB(edges[min_y:max_y, min_x:max_x]))
    axs[1].plot([p0[0]-min_x, p1[0]-min_x], [p0[1]-min_y, p1[1]-min_y], c = 'r')
    axs[1].plot([best_p0[0]-min_x, best_p1[0]-min_x], [best_p0[1]-min_y, best_p1[1]-min_y], c = 'g')
...
def improveEdges(img, edges, plot = False):
    ...
    for p0, p1 in edges:
        ...
        best_p0, best_p1, best_score = optimizePoints(p0_listOrt, p1_listOrt, edgesMatrix)
